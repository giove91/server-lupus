...
class GameTests(TestCase):
    ...
    def test_ipnotista_resurrected(self): # Updated
    ...
    def test_fantasma(self): # Updated
        for i in xrange(20):
            ...
            self.assertTrue(event.ghost in [AMNESIA, CONFUSIONE, CORRUZIONE, ILLUSIONE, OCCULTAMENTO, VISIONE])
    ...
    def test_corruzione_on_medium(self):
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=medium, timestamp=get_now()))
    ...
    def test_corruzione_on_guardia(self):
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=guardia, timestamp=get_now()))
    ...
    def test_corruzione_on_stregone(self):
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=stregone, timestamp=get_now()))
    ...
    def test_scrutatore(self):
        ...
        # Advance to fourth sunset and check that power is not persistent
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.debug_event_bin = []
        ...
        events = [event for event in dynamics.debug_event_bin if isinstance(event, VoteAnnouncedEvent)]
        self.assertEqual(events, [])
        events = [event for event in dynamics.debug_event_bin if isinstance(event, TallyAnnouncedEvent)]
        self.assertEqual(events, [])
    ...
    @record_name
    def test_ipnosi_and_ipnotista(self): # Updated
        ...
        # Use Trasformista power on ex-Ipnotista
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=trasformista, target=ipnotista2, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=trasformista, target=lupo, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=ipnotista1, target=lupo, target2=negromante, timestamp=get_now()))
        ...
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, VoteAnnouncedEvent) and event.voter == trasformista]
        ...
        # Advance to sunset and check that lupo is under trasformista's control
        dynamics.debug_event_bin = []
        test_advance_turn(self.game)
    ...
    @record_name
    def test_ipnosi_on_ipnotista(self):
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=ipnotista, target=trasformista, target2=negromante, timestamp=get_now()))
    ...
    @record_name
    def test_ipnosi_and_amnesia(self):
        roles = [ Ipnotista, Negromante, Lupo, Lupo, Contadino, Contadino ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        # Use powers
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=ipnotista, target=lupo, target2=negromante, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=lupo, timestamp=get_now()))
    ...
    @record_name    
    def test_messia_fails_on_spettri(self): # New
        roles = [ Ipnotista, Negromante, Lupo, Lupo, Contadino, Contadino, Messia, Messia ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        # Advance to third night, kill Ipnotista and ghostify Contadino
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        ...
        self.assertEqual(event1.player, contadino)
        self.assertEqual(event1.ghost, AMNESIA)
        self.assertEqual(event1.cause, NECROMANCER)
        self.assertTrue(isinstance(contadino.role, Spettro))
        self.assertEqual(event2.player, ipnotista)
        self.assertEqual(event2.ghost, IPNOSI)
        self.assertEqual(event2.cause, HYPNOTIST_DEATH)
        self.assertTrue(isinstance(ipnotista.role, Spettro))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=messia1, target=ipnotista, timestamp=get_now()))        
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=messia2, target=contadino, timestamp=get_now()))
    ...
    def test_confusione_visione(self): # New
        ...
        #Check that VISIONE sees a Veggente
    ...
    def test_stregone(self): #New
        roles = [ Negromante, Lupo, Lupo, Sequestratore, Stregone, Contadino ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=sequestratore, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=stregone, target=sequestratore, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=lupo, target=sequestratore, timestamp=get_now()))
