...
class GameTests(TestCase):
    ...
    def test_fantasma(self): # Updated
        for i in xrange(20):
            ...
            self.assertTrue(event.ghost in [AMNESIA, CONFUSIONE, ILLUSIONE, OCCULTAMENTO, VISIONE])
    ...
    def test_preview_dynamics(self):
        roles=[Contadino,Negromante,Lupo]
        self.game = create_test_game(1,roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        preview = dynamics.get_preview_dynamics()
        
        self.assertEqual(negromante.is_mayor(), True)
        self.assertEqual(negromante.is_mayor(preview), False)
        self.assertEqual(negromante.canonicalize(preview).alive, False)
        self.assertEqual(negromante.canonicalize(dynamics).alive, True)
        ...
        self.assertEqual(dynamics.over, False)
        self.assertEqual(preview.over, True)
