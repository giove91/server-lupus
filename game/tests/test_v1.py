...
class GameTests(TestCase):
    ...
    def test_lupo_with_wrong_guardia(self):
        ...
        self.assertEqual(event.visitors, 0)
    ...
    def test_negromante(self):
        ...
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Amnesia, timestamp=get_now()))
        ...
        self.assertIsNotNone(contadino.dead_power)
    ...
    def test_fantasma(self): # Lupus7 update
        for i in range(20):
            ...
            self.assertIsNotNone(fantasma.dead_power)
            ...
            self.assertEqual(event.role_class, Negromante)
            ...
            self.assertTrue(event.role_class in {x for x in phantom_roles})
    ...
    def test_medium_on_spettro(self):
        roles = [ Medium, Negromante, Lupo, Contadino, Contadino ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        dynamics.inject_event(CommandEvent(type=VOTE, player=medium, target=contadino, timestamp=get_now()))
        
        # Advance to second night and make ghost
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        
        # Use powers
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=medium, target=contadino, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Visione, timestamp=get_now()))
        ...
        self.assertEqual(event.target, contadino)
        self.assertEqual(event.role_class, Contadino)
        self.assertTrue(isinstance(contadino.dead_power, Visione))
    ...
    def test_veggente_with_confusione_and_fattucchiera(self): # Lupus7 new
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Confusione, timestamp=get_now()))
    ...
    def test_trasformista_with_stalker(self):
        ...
        self.assertEqual(event.role_class, Stalker)
    ...
    # ~ def test_trasformista_with_fattucchiera(self):
        ...
        # ~ self.assertEqual(event.player, trasformista)
        # ~ self.assertEqual(event.target, fattucchiera)
        # ~ self.assertEqual(event.role_class, Fattucchiera.__name__)
    ...
    def test_trasformista_with_ghostified_investigatore(self):
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=investigatore, role_class=Amnesia, timestamp=get_now()))
        
        # Advance to second night and use power
        test_advance_turn(self.game)
        self.assertIsInstance(investigatore.dead_power, Amnesia)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        ...
        self.assertEqual(event.role_class, Investigatore)
    ...
    def test_necrofilo_with_diavolo(self): # Lupus7 new
        ...
        self.assertEqual(event.role_class, Diavolo)

        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, RoleKnowledgeEvent)]
        self.assertEqual(event.player, diavolo)
        self.assertEqual(event.target, necrofilo)
        self.assertEqual(event.role_class, Necrofilo)
    ...
    def test_voyeur_and_stalker_with_failures(self):
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Occultamento, timestamp=get_now()))
        
        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(contadino.dead_power, Occultamento)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
    ...
    def test_esorcista_with_morte(self):
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=veggente, role_class=Morte, timestamp=get_now()))
        
        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(veggente.dead_power, Morte)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
    ...
    def test_custode_guardia_illusione(self): #Lupus7 new
        roles = [ Custode, Guardia, Negromante, Lupo, Contadino, Contadino, Trasformista ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        [event1, event2] = [event for event in dynamics.debug_event_bin if isinstance(event, QuantitativeMovementKnowledgeEvent)]
        self.assertEqual(event1.visitors, 1)
        self.assertEqual(event2.visitors, 1)
    ...
    def test_voyeur_with_spettro(self):
        ...
        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(contadino.dead_power, Amnesia)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
    ...
    def test_stalker_with_illusione(self):
        ...
        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(contadino.dead_power, Illusione)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
    ...
    def test_voyeur_with_illusione2(self):
        ...
        # Advance to second night and create ghostlupo
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Illusione, timestamp=get_now()))
    ...
    def test_corruzione_on_dead(self): # Lupus7 new
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=veggente, role_class=Corruzione, timestamp=get_now()))

        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(veggente.dead_power, Corruzione)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        ...
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, PowerOutcomeEvent) and event.player is veggente]
        self.assertFalse(event.success)

        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, PowerOutcomeEvent) and event.player is lupo]
        self.assertTrue(event.success)

        self.assertFalse(messia.alive)
        self.assertTrue(isinstance(messia.role, Messia))
        self.assertEqual(messia.team, POPOLANI)

    @record_name
    def test_corruzione_on_dead2(self): # Lupus7 new
        roles = [ Negromante, Lupo, Assassino, Messia, Ipnotista, Veggente, Contadino ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=messia, role_class=Corruzione, timestamp=get_now()))

        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(messia.dead_power, Corruzione)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)

        dynamics.inject_event(CommandEvent(type=USEPOWER, player=assassino, target=lupo, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=veggente, target=lupo, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=messia, target=veggente, timestamp=get_now()))

        # Advance to dawn and check
        dynamics.debug_event_bin = []
        test_advance_turn(self.game)

        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, PowerOutcomeEvent) and event.player is messia]
        self.assertFalse(event.success)

        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, PowerOutcomeEvent) and event.player is assassino]
        self.assertTrue(event.success)
    ...
    @record_name
    def test_voyeur_guardia_with_illusione_of_self(self):
        roles = [ Negromante, Lupo, Lupo, Messia, Ipnotista, Contadino, Voyeur, Guardia, Custode]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=ipnotista, target2=voyeur, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=voyeur, target=ipnotista, timestamp=get_now()))
        ...
        self.assertEqual(event.player, voyeur)
        self.assertEqual(event.target, ipnotista)
        self.assertEqual(event.target2, voyeur)
        self.assertEqual(event.cause, VOYEUR)
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=ipnotista, target2=guardia, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=guardia, target=ipnotista, timestamp=get_now()))
        ...
        self.assertEqual(self.game.current_turn.phase, DAWN)
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, QuantitativeMovementKnowledgeEvent)]
        self.assertEqual(event.player, guardia)
        self.assertEqual(event.visitors, 1)
        self.assertEqual(event.cause, GUARD)
        ...
        dynamics.inject_event(CommandEvent(type=VOTE, player=negromante, target=lupo, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=VOTE, player=lupo, target=lupo, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=VOTE, player=guardia, target=lupo, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=lupo, target2=custode, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=custode, target=lupo, timestamp=get_now()))
        ...
        self.assertEqual(self.game.current_turn.phase, DAWN)
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, QuantitativeMovementKnowledgeEvent)]
        self.assertEqual(event.player, custode)
        self.assertEqual(event.visitors, 1)
        self.assertEqual(event.cause, KEEPER)
    ...
    def test_corruzione_on_messia(self): # Lupus7 new
        ...
        self.assertEqual(event.role_class, Corruzione)
    ...
    @record_name
    def test_corruzione_on_trasformista(self): # Lupus7 new
        roles = [ Negromante, Lupo, Lupo, Contadino, Contadino, Veggente, Mago, Trasformista ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        dynamics.inject_event(CommandEvent(type=VOTE, player=mago, target=veggente, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=VOTE, player=veggente, target=veggente, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=VOTE, player=negromante, target=veggente, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=VOTE, player=lupo, target=veggente, timestamp=get_now()))
        
        # Advance to second night and create ghost, meanwhile kill mago
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=veggente, role_class=Corruzione, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=veggente, target=trasformista, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=trasformista, target=mago, timestamp=get_now()))
        
        # Advance to dawn and check trasformista should succeed but corruzione shouldn't
        dynamics.debug_event_bin = []
        test_advance_turn(self.game)

        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, PowerOutcomeEvent) if event.player is trasformista]
        self.assertTrue(event.success)
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, PowerOutcomeEvent) if event.player is veggente]
        self.assertFalse(event.success)
        
        events = [event for event in dynamics.debug_event_bin if isinstance(event, CorruptionEvent)]
        self.assertEqual(len(events),0)
        self.assertEqual(trasformista.team, POPOLANI)
        self.assertTrue(isinstance(trasformista.role, Mago))
    ...
    def test_corruzione(self): # Lupus7 new
        ...
        self.assertFalse(contadino.alive)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Corruzione, timestamp=get_now()))
    ...
    def test_morte(self): # Update Lupus7
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Morte, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=lupo, target=veggente, timestamp=get_now()))        
    ...
    def test_visione(self): # Lupus7 update
        ...
        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(contadino.dead_power, Visione)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        ...
        self.assertEqual(event.player, contadino)
        self.assertEqual(event.target, ipnotista)
        self.assertEqual(event.role_class, Ipnotista)
    ...
    def test_disjoint_negromanti(self):
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=n1, target=messia, role_class=Amnesia, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=n2, target=contadino, role_class=Amnesia, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=n1, target=contadino, role_class=Illusione, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=n2, target=contadino, role_class=Amnesia, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=n1, target=contadino, role_class=Illusione, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=n2, target=contadino, role_class=Illusione, timestamp=get_now()))
    ...
    def test_disjoint_negromanti_with_sequestratore(self): # New Lupus7
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante1, target=cacciatore, role_class=Illusione, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante2, target=cacciatore, role_class=Visione, timestamp=get_now()))
        ...
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, GhostificationEvent)]
        self.assertEqual(event.player, cacciatore)
        self.assertIsInstance(cacciatore.dead_power, Illusione)
        self.assertEqual(event.ghost, Illusione)
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante1, target=veggente, role_class=Visione, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante2, target=contadino, role_class=Confusione, timestamp=get_now()))
        ...
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, GhostificationEvent)]
        self.assertEqual(event.player, contadino)
        self.assertIsInstance(contadino.dead_power, Confusione)
    ...
    def test_negromante_acts_every_turn(self): # Lupus7 new
        ...
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino1, role_class=Amnesia, timestamp=get_now()))
        ...
        self.assertEqual(event.ghost, Amnesia)
        self.assertEqual(event.cause, NECROMANCER)
        self.assertIsInstance(contadino1.dead_power, Amnesia)
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino2, role_class=Confusione, timestamp=get_now()))
        ...
        self.assertEqual(event.ghost, Confusione)
        self.assertEqual(event.cause, NECROMANCER)
        self.assertIsInstance(contadino2.dead_power, Confusione)
    ...
    @record_name
    def test_scrutatore_dies(self): #Lupus7 update
        roles = [ Negromante, Scrutatore, Lupo, Assassino, Contadino ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        # Use Scrutatore power and kill him with assassino
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=scrutatore, target=negromante, timestamp=get_now()))
        ...
        test_advance_turn(self.game)
        dynamics.debug_event_bin = []
        self.assertFalse(scrutatore.alive)
        test_advance_turn(self.game)
    ...
    def test_scrutatore_and_amnesia(self):
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=scrutatore, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=scrutatore, target=negromante, timestamp=get_now()))
        ...
        self.assertEqual(self.game.current_turn.phase, DAY)
        dynamics.inject_event(CommandEvent(type=VOTE, player=lupo, target=negromante, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=VOTE, player=scrutatore, target=messia, timestamp=get_now()))

        # Advance to sunset that lupo is not voting
        dynamics.debug_event_bin = []
        test_advance_turn(self.game)
        self.assertEqual(self.game.current_turn.phase, SUNSET)
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, VoteAnnouncedEvent)]
        self.assertEqual(event.voter, scrutatore)
        
    def test_scrutatore_and_ipnosi(self):
        roles = [ Negromante, Lupo, Lupo, Messia, Ipnotista, Contadino, Scrutatore ]
        self.game = create_test_game(1, roles)
        dynamics = self.game.get_dynamics()
        players = self.game.get_players()
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino, role_class=Ipnosi, timestamp=get_now()))

        # Advance to third night and use powers
        test_advance_turn(self.game)
        self.assertIsInstance(contadino.dead_power, Ipnosi)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)

        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=scrutatore, target2=messia, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=scrutatore, target=negromante, timestamp=get_now()))
        ...
        events = [event for event in dynamics.debug_event_bin if isinstance(event, VoteAnnouncedEvent)]
        self.assertEqual(len(events),2)
        self.assertEqual(set([e.voted for e in events]),set([messia]))
        self.assertEqual(set([e.voter for e in events]),set([scrutatore, lupo]))

        # Now try ipnosi on lupo
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        ...
        # Now try ipnosi on lupo, but making him vote a different target from scrutatore's
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=contadino, target=lupo, target2=ipnotista, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=scrutatore, target=negromante, timestamp=get_now()))
        ...
        test_advance_turn(self.game)
        self.assertEqual(self.game.current_turn.phase, SUNSET)
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, VoteAnnouncedEvent) and event.voter is scrutatore]
        self.assertEqual(event.voted, messia)
        [event] = [event for event in dynamics.debug_event_bin if isinstance(event, VoteAnnouncedEvent) and event.voter is lupo]
        self.assertEqual(event.voted, ipnotista)
    ...
    def test_ipnosi(self): #update Lupus7
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=mago, role_class=Ipnosi, timestamp=get_now()))
        ...
        self.assertEqual(event.cause, NECROMANCER)
        self.assertIsInstance(mago.dead_power, Ipnosi)
    ...
    def test_ipnosi_on_dead_voter(self): # Update Lupus7
        ...
        self.assertFalse(cacciatore.alive)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=cacciatore, role_class=Ipnosi, timestamp=get_now()))
        ...
        self.assertEqual(event.ghost, Ipnosi)
        self.assertEqual(event.cause, NECROMANCER)
        self.assertIsInstance(cacciatore.dead_power, Ipnosi)
    ...
    def test_many_fantasmi(self): # Lupus7 update
        for i in range(4):
            ...
            for event in events:
                ...
                self.assertEqual(event.cause, PHANTOM)
            self.assertEqual(set([e.ghost for e in events]), {x for x in phantom_roles})
    ...
    def test_amnesia_and_scrutatore_with_quorum(self):
        ...
        self.assertFalse(amnesia.alive)
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=amnesia, role_class=Amnesia, timestamp=get_now()))
    ...
    def test_ipnosi_on_ipnotista(self): #Update Lupus7
        ...
        self.assertEqual(event.ghost, Ipnosi)
        self.assertEqual(event.cause, NECROMANCER)
        self.assertIsInstance(contadino.dead_power, Ipnosi, ipnotista1.role)
    ...
    # def test_trasformista_on_ipnotista_becoming_immune_to_ipnosi(self):
        ...
        # self.assertTrue(ipnotista.role.ghost)
    ...
    # def test_trasformista_on_ipnotista_becoming_immune_to_ipnotista(self):
        ...
        # self.assertEqual(event.ghost, Ipnosi)
        # self.assertEqual(event.cause, HYPNOTIST_DEATH)
        # self.assertTrue(ipnotista1.role.ghost, ipnotista1.role)
    ...
    def test_divinatore(self):
        ...
        dynamics.inject_event(SoothsayerModelEvent(target=cacciatore, advertised_role=Veggente, soothsayer=divinatore, timestamp=get_now()))
        dynamics.inject_event(SoothsayerModelEvent(target=negromante1, advertised_role=Negromante, soothsayer=divinatore, timestamp=get_now()))
        dynamics.inject_event(SoothsayerModelEvent(target=lupo1, advertised_role=Contadino, soothsayer=divinatore, timestamp=get_now()))
        dynamics.inject_event(SoothsayerModelEvent(target=contadino, advertised_role=Contadino, soothsayer=divinatore, timestamp=get_now()))
        # Check
        ...
        self.assertTrue((negromante1, Negromante) in info)
    ...
    def test_divinatore_knowing_about_himself(self):
        ...
        dynamics.inject_event(SoothsayerModelEvent(target=divinatore, advertised_role=Divinatore, soothsayer=divinatore, timestamp=ref_timestamp))
    ...
    def test_divinatore_knowing_about_another_divinatore(self):
        ...
        ref_timestamp = self.game.current_turn.begin
        dynamics.inject_event(SoothsayerModelEvent(target=divinatore2, advertised_role=Divinatore, soothsayer=divinatore1, timestamp=ref_timestamp))

        # Check
        events = [event for event in dynamics.debug_event_bin if isinstance(event, SoothsayerModelEvent)]
        self.assertEqual(len(events), 1)
        event = events[0]
        self.assertEqual(event.soothsayer, divinatore1)

        info = (event.target, event.advertised_role)
    ...
    def test_ipnotista_dies_while_negromanti_create_ipnosi(self): # New
        ...
        self.assertEqual(event.cause, NECROMANCER)
        self.assertIsInstance(contadino.dead_power, Ipnosi)
        self.assertFalse(ipnotista.specter)
    ...
    def test_assassini_act_independently(self): # New
        ...
        for i in range(20):
            ...
            dead = tuple(set([x.role.__class__ for x in players if not x.alive]))
    ...
    def test_confusione(self): # Update Lupus7
        ...
        roles = [event.role_class for event in dynamics.debug_event_bin if isinstance(event, RoleKnowledgeEvent)]
        self.assertEqual(len(set(roles)), 1)
        self.assertEqual(roles[0],Negromante)
    ...
    def test_confusione_visione(self): # Lupus7 update
       ...
       # Advance to first day and kill contadino1
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=lupo, target=contadino2, timestamp=get_now()))
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=contadino1, role_class=Visione, timestamp=get_now()))
        ...
        self.assertEqual(event.player, contadino1)
        self.assertEqual(event.target, negromante)
        self.assertEqual(event.role_class, Veggente)
        self.assertEqual(event.cause, VISION_GHOST)
    ...
    def test_lupi_and_negromanti_not_ghostified(self): # Update Lupus7
        ...
        test_advance_turn(self.game)
        test_advance_turn(self.game)
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=diavolo, role_class=Amnesia, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=assassino, role_class=Amnesia, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=lupo2, role_class=Amnesia, timestamp=get_now()))
        ...
        dynamics.inject_event(CommandEvent(type=USEPOWER, player=negromante, target=medium, role_class=Amnesia, timestamp=get_now()))
